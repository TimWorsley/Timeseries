...
function getMonthlyAverageShapes(ts) {
    ...
    for (var monthIndex in ts.monthly.time) {
        var month = {
            time: ts.monthly.time[monthIndex],
            averageOpen: ts.monthly.averageOpen[monthIndex],
            averageClose: ts.monthly.averageClose[monthIndex]
        };
...
function RenderReport(reportId, volumeId, ts) {
// define base report
// add monthly lines
// add best day
// define volume report, while painting some values red
    ...
    for (var dayIndex in volumeData.x ) {
        if (volumeData.y[dayIndex] > highVolume) {
...
//    var volumeData = [getVolumeData(key, reebo)];
    ...
    Plotly.plot(volumeId, [volumeData], {'title': ts.symbol + ' Volume, red for 10% above average of ' + prettyAverage, 'height': 400});
};
